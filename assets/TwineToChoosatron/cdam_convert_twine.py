...
def main():
	...
	if args.binary == True:
		...
		for index in range(1, len(PASSAGES) + 1):
			...
			if p['en'] == True:
				if p['eq'] == 1:
					book += "\n\n\t* Oh no! Better luck next adventure. * - THE END"
				elif p['eq'] == 2:
					book += "\n\n\t** I'm sure you can do better. ** - THE END"
				elif p['eq'] == 3:
					book += "\n\n\t*** You win some, you lose some. *** - THE END"
				elif p['eq'] == 4:
					book += "\n\n\t**** Not too bad! **** - THE END"
				elif p['eq'] == 5:
					book += "\n\n\t***** Congratulations! You sure know your stuff. ***** - THE END"
			else:
				if len(p['cs']) == 1:
					book += ("\n- " + p['cs'][0] + goto + m[0] + ")")
				else:
