...
void CSN_Thermal::setTimes(unsigned long p, unsigned long f) {
  dotPrintTime = p;
  dotFeedTime  = f;
}

// Constructor
CSN_Thermal::CSN_Thermal() {
}

void CSN_Thermal::init() {
  prevByte = 0;
  column = 0;
  maxColumn = 0;
  charHeight = 0;
...
void CSN_Thermal::writePrintMode() {
  writeBytes(27, 33, printMode);
}

void CSN_Thermal::normal() {
  printMode = 0;
  writePrintMode();
}

void CSN_Thermal::inverseOn(){
  setPrintMode(INVERSE_MASK);
}

void CSN_Thermal::inverseOff(){
  unsetPrintMode(INVERSE_MASK);
}

void CSN_Thermal::upsideDownOn(){
  setPrintMode(UPDOWN_MASK);
}

void CSN_Thermal::upsideDownOff(){
  unsetPrintMode(UPDOWN_MASK);
}

void CSN_Thermal::doubleHeightOn(){
  setPrintMode(DOUBLE_HEIGHT_MASK);
}

void CSN_Thermal::doubleHeightOff(){
...
void CSN_Thermal::boldOff(){
  unsetPrintMode(BOLD_MASK);
}

void CSN_Thermal::justify(char value){
  uint8_t pos = 0;
...
// 1 - normal underline
// 2 - thick underline
void CSN_Thermal::underlineOn(uint8_t weight) {
  writeBytes(27, 45, weight);
}

void CSN_Thermal::underlineOff() {
  underlineOn(0);
}
...
void CSN_Thermal::printBitmap(int w, int h, Stream *stream) {
...
void CSN_Thermal::printBitmap(Stream *stream) {
  uint8_t  tmp;
...
void CSN_Thermal::offline(){
  writeBytes(27, 61, 0);
}

// Take the printer back online. Subsequent print commands will be obeyed.
void CSN_Thermal::online(){
  writeBytes(27, 61, 1);
}

// Put the printer into a low-energy state immediately.
void CSN_Thermal::sleep() {
  sleepAfter(1);
}

// Put the printer into a low-energy state after the given number
// of seconds.
void CSN_Thermal::sleepAfter(uint8_t seconds) {
  writeBytes(27, 56, seconds);
}

// Wake the printer from a low-energy state.
void CSN_Thermal::wake() {
...
bool CSN_Thermal::hasPaper() {
  ...
  for (int i = 0; i < 1000; i++) {
    if (SERIAL_IMPL.available()) {
      stat = SERIAL_IMPL.read();
      break;
    }
  }
...
////////////////////// not working?
void CSN_Thermal::tab() {
  PRINTER_PRINT(9);
}
void CSN_Thermal::setCharSpacing(int spacing) {
